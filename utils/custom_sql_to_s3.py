...
from utils.rdbms import get_pandas_from_mysql
    ...
    def _fix_dtypes(df: DataFrame, file_format: FILE_FORMAT) -> None:
        ...
        for col in df:
            ...
            if df[col].dtype.name == "object" and file_format == "parquet":
                ...
                print(f'check1 : {df[col].dtype.name}')

            if df[col].hasnans:
                # inspect values to determine if dtype of non-null values is
                # int or float

                print(f'check2 : {df[col].dtype.name}')
                try:
                    ...
                    if np.equal(notna_series, notna_series.astype(int)).all():
                        ...
                        print(f'check3 : {df[col].dtype.name}')
                        ...
                        print(f'check4 : {df[col].dtype.name}')
                except Exception as e:
                    print(f'error : [{e}]')

            print(f'check5 : {df[col].dtype.name}')
            print('----' * 100)

    def execute(self, context: Context) -> None:
        sql_hook = self._get_hook()
        s3_conn = S3Hook(aws_conn_id=self.aws_conn_id, verify=self.verify)
        data_df = get_pandas_from_mysql(sql_hook, self.query)
        print('-' * 100)
        print(data_df.info())
        print('-' * 100)
